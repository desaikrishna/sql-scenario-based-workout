...
with cte_1 as(
	select b.*,extract(epoch from (session_endtime - session_starttime)) as total_time 
	from user_sessions a
	join post_views b
on a.session_id = b.session_id)
select post_id, sum((perc_viewed/100)*total_time) as total_view_time from cte_1
group by post_id
