...
cross join(select length from footer where length is not null order by id limit 1) length
cross join(select width from footer where width is not null order by id limit 1) width
cross join(select height from footer where height is not null order by id limit 1) height;