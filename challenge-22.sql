...
with cte_teams as(
	...
	cte as(
	select dates,concat(home_team,' vs ', away_team) as match, teams,result
	...
	--where teams='Lucknow Super Giants'
	--order by teams, dates
		),
	cte_diff as(
	select *
	, row_number() over(partition by teams order by teams,dates) temp_rn
	, id - row_number() over(partition by teams order by teams, dates) as diff
	from cte
	where result=teams
	),
	cte_final as(
	select *, count(1) over(partition by teams order by teams, dates
...
from cte_teams t
left join cte_final f on t.teams=f.teams
group by t.teams
order by max_winning_streak desc

/*
union helps us to get total teams in ipl that is 10 teams

cte: matches played by each team that is the joun condition
then you order it based on team and date
row_number is added to get unique number each team so partition it by teams

cte_diff: cte has 1-16(total matches played by a team) row_number displayed, so we need logic wherein
it if name of team is same in result we get same number
result=teams, team that has won

cte_final: will give count of streak
parition is done by team, diff because we need it for each team and the streak(same number it has)


wherever the number is sequential(temp_rn) if subtracted with same row number(id) we will get same value
then count same number and take max out of it
...
If you run this updates the winning streak should change in your query
