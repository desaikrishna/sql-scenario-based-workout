...
with cases_monthly as(
	select extract(month from dates) as month,sum(cases_reported) as monthly_cases
	from covid_cases
...
select *, case when month >1 
				then round(((monthly_cases/lag(total_cases) over(order by month)) *100),2)::varchar
